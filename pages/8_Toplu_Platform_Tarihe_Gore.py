...
def read_csv_auto(uploaded_file) -> pd.DataFrame:
    """CSV'yi önce sep=';' ile oku, olmazsa otomatik sep ile dene."""
...
def detect_datetime_columns(df: pd.DataFrame, min_ratio: float = 0.05) -> List[str]:
    dt_cols = []
    for col in df.columns:
        if pd.api.types.is_datetime64_any_dtype(df[col]):
            dt_cols.append(col)
        elif df[col].dtype == "O" or pd.api.types.is_string_dtype(df[col]):
            parsed = pd.to_datetime(df[col], errors="coerce", dayfirst=True, infer_datetime_format=True)
            if parsed.notna().sum() / len(parsed) >= min_ratio:
                dt_cols.append(col)
    # isimde tarih / date geçenleri başa al
    return sorted(dt_cols, key=lambda c: (0 if any(k in c.lower() for k in ["tarih", "date"]) else 1, c))
...
selected_dt_col = st.selectbox("Datetime kolonu seç:", candidate_dt_cols)
...
with col1:
    selected_sources = st.multiselect("Kaynak seç (default hepsi):", sorted(df["kaynak"].unique()), default=list(sorted(df["kaynak"].unique())))
with col2:
    start_date, end_date = st.date_input("Başlangıç / Bitiş", value=(min_date, max_date), min_value=min_date, max_value=max_date)
